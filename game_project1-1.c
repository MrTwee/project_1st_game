...
void shuffle(char Pair_game_table[ROWS][COLS][CARD_ROWS][CARD_COLS]) {
    ...
    for (int i = NUM_PAIRS * ROWS * COLS - 1; i > 0; i--) {
        int j = rand() % (i + 1);
        for (int l = 0; l < CARD_ROWS; l++) {
            swap(Pair_game_table[j / (COLS * CARD_ROWS)][(j % (COLS * CARD_ROWS)) / CARD_ROWS][l],
                 Pair_game_table[i / (COLS * CARD_ROWS)][(i % (COLS * CARD_ROWS)) / CARD_ROWS][l]);
        }
    }
}
